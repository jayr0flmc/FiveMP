...
Hash GetHashFromModelName(char* modelName) {
	return GAMEPLAY::GET_HASH_KEY(modelName);
}

Hash GetModelHashFromModelID(int modelid) {
	return GetHashFromModelName(pedModels[modelid]);
}
...
void UpdatePedModel(int playerid) {
	...
	if (STREAMING::IS_MODEL_IN_CDIMAGE(playerData[playerid].pedModel) && STREAMING::IS_MODEL_VALID(playerData[playerid].pedModel))
		...
		STREAMING::REQUEST_MODEL(playerData[playerid].pedModel);
	while (!STREAMING::HAS_MODEL_LOADED(playerData[playerid].pedModel))
		WAIT(0);
	playerData[playerid].pedPed = PED::CREATE_PED(playerData[playerid].pedType, playerData[playerid].pedModel, playerData[playerid].x, playerData[playerid].y, playerData[playerid].z, 0.0f, false, true);
	STREAMING::SET_MODEL_AS_NO_LONGER_NEEDED(playerData[playerid].pedModel);

	ENTITY::SET_ENTITY_NO_COLLISION_ENTITY(LocalPlayer->playerPed, playerData[playerid].pedPed, false);
	ENTITY::SET_ENTITY_NO_COLLISION_ENTITY(playerData[playerid].pedPed, LocalPlayer->playerPed, false);
